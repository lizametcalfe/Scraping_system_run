...
#this needs to be in square brackets so that is runs from Crontab - it won't run the R code otherwise
#mkae sure the whole paths are specified otherwise this will not work
import subprocess
command = 'RScript'
subprocess.call(['/usr/bin/Rscript', '--vanilla', '/media/mint/e834712c-23da-4cbe-a4ec-3a35d416877b/Run_system/WebScrapingSystem/identify_missing_v3.R'])
...
from functions.add_to_databases import combine   
combine()

#os.system("python /media/mint/e834712c-23da-4cbe-a4ec-3a35d416877b/Run_system/functions/add_to_databases.py")

print("Added to _main_ database")
#add todays data to the mongo database

#    subprocess.call(['/media/mint/e834712c-23da-4cbe-a4ec-3a35d416877b/Run_system/functions/load_csv_single.sh']) 
...
#the code below will auotmate the process from Python however, we are using Contrb instead (open a terminal from the highest folder (run_system) and type contrab -e.)
