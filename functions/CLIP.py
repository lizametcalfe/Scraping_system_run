...
try:
    os.remove("/media/mint/e834712c-23da-4cbe-a4ec-3a35d416877b/Run_system/Data/price_indices/CLIP2014.csv")
    os.remove("/media/mint/e834712c-23da-4cbe-a4ec-3a35d416877b/Run_system/Data/price_indices/CLIP2015.csv")
    os.remove("/media/mint/e834712c-23da-4cbe-a4ec-3a35d416877b/Run_system/Data/price_indices/CLIP2016.csv")
except:
    pass
...
def basetree(data, basedate):
    ...
    del data4["price"]
    print("mean shift labels")
    print(ward.labels_)

    #fit decision tree clusters
    dt = dt.fit(np.array(data4),ward.labels_+1)
...
def geo(results,base):
    ...
    table.sort_values('cluster', inplace=True, axis = 0)
    table = pd.merge(base, table, how='left', on='cluster')
    print(table)
    #print("number of clusters")
    #print(len(table["cluster"]))p
    table.loc[:,"pr"] = table.loc[:,'gmean_y']/table.loc[:,'gmean_x']
    table.loc[:,"pr"] = table["pr"].fillna(1)
    table.loc[:,"pr"] = table.apply(lambda x: 1 if x["count_y"] < (x["count_x"]/4) else x["pr"],axis=1)
    table.reset_index()
...
def jevons(data,date,basedate):
    data_base_prices = data.loc[data['monthday'].astype(int) == basedate][['product_name','price']]
    data_base_prices.rename(columns={'price':'base_price'}, inplace=True)
    data_comp_prices = data.loc[data['monthday'].astype(int) == date][['product_name','price']]
    datamerged=pd.merge(data_base_prices,data_comp_prices, how='inner', on='product_name')
    datamerged.loc[:,'price_relative'] = datamerged.loc[:,'price']/datamerged.loc[:,'base_price']
    datamerged.loc[:,'pr_log'] = datamerged['price_relative'].apply(math.log)
    datamerged.loc[:,"groups"] = 1
...
##    print(len(uniquedata)/100)*nn
    if len(uniquedata) < 30:
        ...
        try:
        	...
        	new =100
        	print("not enough data to jevons!")
        print("jevons")
        bbb["type"] = "Jevons"
        ...
        if bt.n_classes_ <=2:
            ...
            try:
                ...
                if date == basedate:
                    ...
                    basestructure.to_csv("/media/mint/e834712c-23da-4cbe-a4ec-3a35d416877b/Run_system/Data/basestructure.csv")
                    basedclust = applydecisiontree(data2,basedate,pd.read_csv("/media/mint/e834712c-23da-4cbe-a4ec-3a35d416877b/Run_system/Data/basestructure.csv"))
                    basedclust.to_csv("/media/mint/e834712c-23da-4cbe-a4ec-3a35d416877b/Run_system/Data/basedclust.csv")
                clust = applydecisiontree(data2,date,pd.read_csv("/media/mint/e834712c-23da-4cbe-a4ec-3a35d416877b/Run_system/Data/basestructure.csv"))
                new = pd.DataFrame()
                new = geo(clust, geobase(pd.read_csv("/media/mint/e834712c-23da-4cbe-a4ec-3a35d416877b/Run_system/Data/basedclust.csv")))
                bbb["type"] = "CLIP"
            except:
                try:
                    new = jevons(pd.DataFrame(data),date,basedate)
            ...
            new = "wrong"
            print("wrong")
    ab= pd.read_csv("/media/mint/e834712c-23da-4cbe-a4ec-3a35d416877b/Run_system/Data/metadata.csv")
    ab = ab.append(bbb)
    print(ab)
    del ab["Unnamed: 0"]
    ab.to_csv("/media/mint/e834712c-23da-4cbe-a4ec-3a35d416877b/Run_system/Data/metadata.csv")
    return new
...
def CLIPPEDdate(data, idvar,classvar,pricevar,datevar,basedate):
    ...
    df1.index = range(T)
    try:
        os.remove("/media/mint/e834712c-23da-4cbe-a4ec-3a35d416877b/Run_system/Data/basestructure.csv")
        os.remove("/media/mint/e834712c-23da-4cbe-a4ec-3a35d416877b/Run_system/Data/basedclust.csv")
    except:
        pass
    return df1

####monthly (updated) ########
x = pd.read_csv('/media/mint/e834712c-23da-4cbe-a4ec-3a35d416877b/Run_system/Data/averages/_month_.csv',encoding="latin-1")
x.offer = x.offer.str.encode('utf-8')
x = x[x["item_price_num"]>0]
x.loc[:,"ons_item_name"] = x["ons_item_name"].apply(lambda x: x.strip())

x = x[x["ons_item_name"]!="fizzy bottled drink"]
x = x[x["ons_item_name"]!="potatoes, baking"]
...
x=x[x['item_price_num']>0]
x.loc[:,"idvar"]=x["product_name"]+"_"+x["store"]

x =x.reset_index()

x = man(x,"month")

x14 = x[x["monthday"]<201502]
x15 = x[x["monthday"]>=201501]
x15 = x15[x15["monthday"]<201602]
x16 = x[x["monthday"]>=201601]
...
aa=pd.DataFrame()
a = runthrough(x14, 201406)
a.to_csv("/media/mint/e834712c-23da-4cbe-a4ec-3a35d416877b/Run_system/Data/price_indices/CLIP2014.csv")

b = runthrough(x15, 201501)
b.to_csv("/media/mint/e834712c-23da-4cbe-a4ec-3a35d416877b/Run_system/Data/price_indices/CLIP2015.csv")

c = runthrough(x16, 201601)
c.to_csv("/media/mint/e834712c-23da-4cbe-a4ec-3a35d416877b/Run_system/Data/price_indices/CLIP2016.csv")
